...
Если же ответа в памяти нет, то метапоиск передаёт запрос на сервера другой компьютерной системы — базового поиска(**BaseSearch**). Базовый поиск ищет ответ на запрос в поисковом индексе (**ElasticSearch**), а если нужно обращается к слепку интернета, который храниться в храниться в  map-reduce хранилище(**YTsaurus**).</br>
В результате отдаётся список документов, в которых есть слова из запроса, в систему Матрикснет(**MatrixNet**) Там они объединяются, радируются и попадают на страницу результатов поиска. Благодаря такой организации поиск Яндекса может отвечать пользователю за доли секунды.

__Поисковые боты__</br>
Формируют поисковый индекс 2 поисковых робота — основной(**SlowRobot**) и быстрый(**FastRobot**). Основной робот индексирует интернет в целом, а быстрый отвечает за то, чтобы в поиске можно было найти самые свежие документы. У каждого робота есть список адресов документов, которые нужно проиндексировать, которые формирует программа-планировщик(**Planer**).
