...
__Ведомость__ - __[Таблица отчетности](https://vk.com/away.php?to=https%3A%2F%2Fdocs.google.com%2Fspreadsheets%2Fd%2F1XhRgeQARIXGoiTSYQStW4GTlXJ62-jgL36c2i8HvYJ4%2Fedit%3Fusp%3Dsharing&cc_key=)</br>
...
Среднее количество действий пользователя можно оценить следующим образом. Т.к. данный сервис представляет собой поисковую систему, то, вероятнее всего, что в общем случае сценарий использования сервиса следующий: пользователи заходят на главную поисковую страницу, набирают запрос и пользуются пагинатором. Согласно [5], страниц за визит 5,8 значит, 3-4 перехода пользователь проводит за сеанс использования ресурса.</br>
...
_RPS на авторизацию_ найдем из следующих соображений, поделим DAU на секунды в дне.<br>
...
Исходя из данных, представленных в первом пункте курсовой работы, можно сделать вывод, что нужно расположить большую часть дата-центров в России.<br>

Так как население России распределено неравномерно, нам потребуется расположить основную часть дата-центров в европейской части страны[14].<br>
...
Следовательно, были выбраны следующие города для установки дата-центров:
...
- Применим latency-based DNS, так как он позволяет эффективно распределять трафик по RTT к ближайшим серверам
...
Воспользуемся данными, полученными из 2-го раздела.</br>
Для таблицы _Users_ запросы на чтение примем как запросы на авторизацию, запросы на запись примем равными 5.</br>
Для таблиц _History_ запросы на чтение примем рассчитаем следующим образом, предположим, что история пользователя синхронизируется (как пишется, так и считывается) 1 раз в день. Таким образом, количество запросов равно количеству запросов на авторизацию.</br>
...
__Шардинг__
Шардинг является основой построения системы так как, хранимое количество данных очень велико и физически не уместится на 1-й СХД. Поэтому индекс и слепок интернета равномерно распределяется по дата-центрам, учитывая ранее приведенное расположение дата-центров.

__Репликация__
Реплицировать имеет смысл только индекс и данные пользователей. Потеря реплики интернета не приведет к существенным проблемам так как, этот слепок сети несет лишь вспомогательную функцию.
...
Кроме того, учитываются при поиске различные потребности пользователей. У каждой категории есть список возможных потребностей — тех намерений, с которыми пользователи ищут тот или иной объект, например слово "купить". Всего у категории может быть от двух-трех до нескольких десятков потребностей.<br>
...
------------------|----------------------------------|---------------------------
 ...
 Postgres         | БД пользователи, история поиска  | самая популярная БД
...
---------------------------|--------------------------
 ...
 4.Postgres                | Репликация данных
 5.ElasticSearch           | Репликация данных
 6.YTsaurus                | Репликация данных

1. Резервировать ДЦ будем с использованием дублирования всех важных компонент в физически разнесенные железные ресурсы. В случае отказа ДЦ трафик будет балансироваться latency-base DNS. Также будет проводиться на местах физическое резервирование компонент (например, диски и другие компоненты для горячей замены).
1. Локальная балансировка будет проводиться с использованием BGP маршрутизации на уровне L3. А на уровне L7 будет проводиться балансировка Nginx с помощью Weighted Least Connections (рафик распределяется между серверами на основе количества активных соединений и весов, назначенных каждому серверу. Также будут эксплуатироваться механизмы Kubernetes для автоматического перезапуска контейнеров в случае их сбоя.
1. Репликация Redis не требуется, так как данная БД не содержит критических данных, и в случае необходимости будет проведена замена её резервной БД.
1. В Postgres будет использоваться асинхронная потоковая репликация (Asynchronous Streaming Replication). Streaming Replication — это репликация, при которой от основного сервера PostgreSQL на реплики передается WAL (журнал предзаписи транзакций). И каждая реплика затем по этому журналу изменяет свои данные. Асинхронность репликации выражается в том, что сначала применит изменения на основном узле и только потом отправит записи из WAL на реплики. Преимущество такого способа — быстрое подтверждение транзакции, т. к. не нужно ждать, пока все реплики применят изменения. Недостаток в том, что при падении основного сервера часть данных на репликах может потеряться, так как изменения не успели продублироваться. Учитывая особенность применения Postgres в данном случае, этот недостаток не является критическим, так как основная нагрузка на сервер будет представлять из себя записи истории пользователей, и небольшие потери этих записей не принесут серьезных последствий.
...
Также имеются механизмы для создания шардов (_Cluster-level Sharding_) и бэкапов (_Failover и Auto-discovery_).
1. YTsaurus изначально создавался как распределенная система и поддерживает различные механизмы отказоустойчивости, которые реализуются на уровне файловой системы. В случае YTsaurus это Кипарис (Cypress) — древовидный отказоустойчивый сторадж. Основа Кипариса — реплицированный и горизонтально-масштабируемый мастер-сервер, который хранит метаинформацию об устройстве дерева Кипариса, а также о составе и местоположении реплик чанков всех таблиц на кластере.

__Обеспечение надежности сервиса__

Существуют несколько принципов, которых стоит придерживаться при проектировании системы, для поддержания её высокой отказоустойчивости:

* _Graceful shutdown_ - это принцип обеспечения надежности веб-сервиса, который позволяет безопасно завершить работу сервиса, минимизируя потерю данных и обеспечивая плавный переход к состоянию выключения. В Kubernetes предоставляет возможность управления жизненным циклом приложений. При остановке или масштабировании приложения Kubernetes может использовать стратегии завершения (Termination Grace Period), чтобы позволить приложению завершить текущие запросы и сохранить состояние перед полным выключением. Но при этом стоит понимать, что поисковые запросы во время обработки нигде не кешируются и не сохраняются. И в случае, если пода Kubernetes не успеет обработать запрос, то он будет потерян, однако это не так критично. Достаточно вспомнить свой пользовательский опыт использования браузера, при необходимости пользователем будет отправлен дополнительный запрос (например, нажатием F5), и потеря запроса не критична.

* _Failover policy_ -  принцип обеспечения надежности веб-сервиса, который предусматривает автоматическое переключение на резервные ресурсы или серверы в случае сбоя или недоступности основных ресурсов. Это можно достичь за счет репликации сервисов средствами Kubernetes. Kubernetes поддерживает автоматическое переключение на резервные ресурсы или серверы в случае сбоя или недоступности основных ресурсов. Когда один из подов или узлов выходит из строя, Kubernetes автоматически перезапускает под на другом доступном узле, чтобы обеспечить непрерывность работы приложения. Учитывая это и структуру поиска, единственным механизмом обеспечения данного принципа будет резервирование и реплицирование сервисов. Сервисов структурно схожего или дублирующегося функционала не предусмотренно.

* _Graceful degradation_ - принцип обеспечения надежности веб-сервиса, который позволяет поддерживать работоспособность сервиса даже при возникновении проблем или сбоев. Частичную функциональность системы можно будет поддержать при выходе из строя системы Базового поиска(*BaseSearch*) так, как часть ответов на популярные поисковые запросы сохранится в Метапоиске (*MetaSearch*), но система существовать не сможет так, как вся обработка запроса конвейерная (последовательная).
...
Как говорилось ранее вся система состоит из нескольких больших компонент:

- Пользовательский поиск - отвечает на поисковые запросы пользователей;
- Поисковые боты - обходят интернет, благодаря чему формируют индекс и реплику всемирной сети;
- YTsaurus - распределенная файловая система для хранения реплики интернета;
- ElasticSearch - распределенная БД для хранения поискового индекса.

__Пользовательский поиск__</br>

Пользовательские запросы попадают в метапоиск(**MetaSearch**). Метапоиск обрабатывает каждый запрос в реальном времени — выясняет все необходимые данные про запрос (из какого региона он был задан, к какому классу относится и т.п.), проводит лингвистическую обработку. Затем метапоиск проверяет, формировались ли в последнее время результаты поиска для этого запроса. Результаты поиска по часто задаваемым запросам некоторое время хранятся в памяти метапоиска, благодаря кешированию в **PostgreSQL**.</br>
Если же ответа в памяти нет, то метапоиск передаёт запрос на сервера другой компьютерной системы — базового поиска(**BaseSearch**). Базовый поиск ищет ответ на запрос в поисковом индексе (**ElasticSearch**), а если нужно обращается к слепку интернета, который храниться в map-reduce хранилище(**YTsaurus**).</br>
В результате отдаётся список документов, в которых есть слова из запроса, в систему Матрикснет(**MatrixNet**) Там они объединяются, ранжируются и попадают на страницу результатов поиска. Благодаря такой организации поиск Яндекса может отвечать пользователю за доли секунды.

__Поисковые боты__</br>
Формируют поисковый индекс 2 поисковых робота — основной(**SlowRobot**) и быстрый(**FastRobot**). Основной робот индексирует интернет в целом, а быстрый отвечает за то, чтобы в поиске можно было найти самые свежие документы. У каждого робота есть список адресов документов, которые нужно проиндексировать, их формирует программа-планировщик(**Planer**).
...
Предположим, что бэкенд на Go удовлетворяет следующим требованиям:
...
Оценивать стоимость СХД довольно проблематично, так как цена существенно зависит от конфигурации системы. Основным критерием выбора будет возможность масштабирования вычислительного кластера, максимально поддерживаемый размер дисков, а также поддержка HTTP и HDFS. Рассмотрим продукцию отечественного производителя серверного оборудования — Ядро. Они являются разработчиками серверного комплекса ЯДРО [TATLIN.OBJECT](https://yadro.com/ru/tatlin/object/specs), который поддерживает до 50 Петабайт хранилище. Этого будет недостаточно для хранения реплики интернета, но достаточно для хранения индекса и данных пользователя. Для HDFS выберем более серьезный комплекс [Huawei OceanStor 9000 V5 Scale-Out NAS](https://e.huawei.com/en/products/storage/scale-out-storage/9000), который поддерживает до 150 Петабайт данных.
